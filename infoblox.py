...
class infoblox(object):
	...
	def __init__(self, callback=None, auth=None, vers='v1.7.1'):
		...
		input		callback (funct)	An optional callback can be passed at instantiation for error and logging purposes
		output		void (void)
		"""
	...
	def __del__(self):
		"""
		class destructor - Invalidates the cookie on the Infoblox side to effectively log the user out
		...
		while(1):
			...
			creds = base64.b64encode('{0}:{1}'.format(user, passwd))
		        with warnings.catch_warnings():
                		...
                		resp = requests.get('https://{0}/wapi/{1}/record:host?name~={0}'.format(url, self.vers), headers={'Authorization' : 'Basic {0}'.format(creds), 'Accept' :'application/xml'}, verify=False)
	...
	def get(self, query):
		...
		with warnings.catch_warnings():
			warnings.simplefilter("ignore")
			return requests.get('https://{0}/wapi/{1}/{2}'.format(self.url, self.vers, query), headers={'Authorization' : 'Basic {0}'.format(self.creds), 'Accept' :'application/json'}, verify=False)
	...
	def post(self, api_function, payload):
		...
		with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
			return requests.post('https://{0}/wapi/{1}/{2}'.format(self.url, self.vers, api_function), data=payload, headers={'Authorization' : 'Basic {0}'.format(self.creds)}, verify=False)
	...
	def put(self, api_function, payload):
		...
		with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
			return requests.put('https://{0}/wapi/{1}/{2}'.format(self.url, self.vers, api_function), data=payload, headers={'Authorization' : 'Basic {0}'.format(self.creds)}, verify=False)
	...
	def delete(self, api_function):
		...
		with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
			return requests.delete('https://{0}/wapi/{1}/{2}'.format(self.url, self.vers, api_function), headers={'Authorization' : 'Basic {0}'.format(self.creds)}, verify=False)
	...
	class _host(object):
		...
		def fetch(self):
			...
			resp = self.infoblox_.get('record:host?name~={0}'.format(self.hostname))
	                if resp.status_code != 200:
	                        try:
	                                return self.infoblox_.__caller__('Could not retrieve host _ref for {0} - Status {1}'.format(self.hostname, resp.status_code), resp.status_code)
	                        except:
		...
		def add(self, ip, mac=None):
			...
			"""
			if mac != None:
				payload = '{{"name":"{0}", "ipv4addrs":[ {{"ipv4addr":"{1}","mac":"{2}"}}]}}'.format(self.hostname, ip, mac)
			else:
				payload = '{{"name":"{0}", "ipv4addrs":[ {{"ipv4addr":"{1}"}}]}}'.format(self.hostname, ip, mac)
			resp = self.infoblox_.post('record:host',payload)
	                if resp.status_code != 201:
				try:
	                		return self.infoblox_.__caller__('Error creating host record {0} for {1} - Status: {2}'.format(ip, self.hostname, resp.status_code), resp.status_code)
				except:
					return resp.status_code
			return 0
		...
		def delete(self):
			...
			if resp.status_code != 201:
	                	try:
	                		return self.infoblox_.__caller__('Error creating host record {0} for {1} - Status: {2}'.format(self.hostname, resp.status_code), resp.status_code)
	                	except:
	                        	return resp.status_code
	                return 0
		...
		class _alias(object):
			...
			def fetch(self):
				...
				"""
				resp = self.infoblox_.get('record:host?_return_fields%2B=aliases&name={0}'.format(self.host_.hostname))
	                	if resp.status_code != 200:
	                	        try:
	                	                return self.infoblox_.__caller__('Could not retrieve aliases for {0} - Status {1}'.format(self.host_.hostname, resp.status_code), resp.status_code)
	                	        except:
	                	                return resp.status_code
	                	try:
			...
			def add(self, new_alias):
				...
				if resp.status_code != 200:
					try:
						return self.infoblox_.__caller__('Could not set aliases for {0} - Status {1}'.format(self.host_.hostname, resp.status_code), resp.status_code)
					except:
						return resp.status_code
				return 0
			...
			def delete(self, rm_alias):
				...
				resp = self.infoblox_.put('{0}'.format(self.host_._ref), payload)
		                if resp.status_code != 200:
		                        try:
		                                return self.infoblox_.__caller__('Could not unset aliases for {0} - Status {1}'.format(self.host_.hostname, resp.status_code), resp.status_code)
		                        except:
		                                return resp.status_code
		                return 0
	...
	class _grid(object):
		...
		def _ref(self):
			...
			if resp.status_code != 200:
				try:
	        			return self.infoblox_.__caller__('Could not get grid _ref - Status {0}'.format(resp.status_code), resp.status_code)
				except:
					return resp.status_code
		...
		def restart(self):
			...
			output		0 (int)			Success
					1 (int)			Failure
			"""
			resp = self.infoblox_.post('{0}?_function=restartservices&member_order=SEQUENTIALLY&sequential_delay=10&service_option=ALL&restart_option=FORCE_RESTART'.format(self._ref),'')
			if resp.status_code != 200:
				try:
					return self.infoblox_.__caller__('Could not restart Infoblox gridmaster - Status {0}'.format(resp.status_code), resp.status_code)
				except:
					return resp.status_code
			return 0
	...
	class _subnet(object):
		...
		def _ref(self):
			...
			if resp.status_code !=200:
				try:
					return self.infoblox_.__caller__('Error getting subnet ID for subnet {0} - Status {1}'.format(self.subnet, resp.status_code), resp.status_code)
				except:
					return resp.status_code
			return json.loads(resp.text)[0]['_ref']
		...
		def next_available_ip(self, offset=2):
			"""
			next_available_ip - Get the next available IP address in a subnet. The first results is always the gateway.
			
			input		offset (int)		Optional arg to provide address offset for networking gear/etc not accounted in IPAM
			output		ip_addr (string)	IP address
					None (null)		No free IP addresses
			"""
			payload = '{{"num":{0}}}'.format(offset)
			resp = self.infoblox_.post('{0}?_function=next_available_ip'.format(self._ref), payload)
			if resp.status_code != 200:
				try:
					return self.infoblox_.__caller__('Error retrieving next available address - Status {0}'.format(resp.status_code), resp.status_code)
				except:
					return resp.status_code
			try:
		...
		def prompt(self):
			"""
			prompt - Prompt the user for a list of allowed subnets to assign IP Adresses for sandboxes
		...
		def subnet_format_check(self, l_subnets):
			"""
			subnet_format_check - Checks to make sure the subnet is properly formatted in CIDR notation
	...
	class _cname(object):
		...
		def __init__(self, infoblox_, name):
			...
			self._ref = self._ref()
                ...
                def fetch(self):
                        ...
                        if resp.status_code != 200:
                                try:
                                        return self.infoblox_.__caller__('Could not retrieve CNAME _ref for {0} - Status {1}'.format(self.name, resp.status_code), resp.status_code)
                                except:
                                        return resp.status_code
                        try:
                                return json.loads(resp.text)[0]
		...
		def add(self, canonical, ttl=None):
			...
			"""
			if ttl != None:
				payload = '{{"name":"{0}","canonical":"{1}","ttl":{2}}}'.format(self.name, canonical,ttl)
			else:
				payload = '{{"name":"{0}","canonical":"{1}"}}'.format(self.name, canonical)
			resp = self.infoblox_.post('record:cname',payload)
 			if resp.status_code != 201:
                                try:
                                        return self.infoblox_.__caller__('Could not create CNAME record for {0} - Status {1}'.format(self.name,resp.status_code), resp.status_code)       
				except:
                                	return resp.status_code
                        return 0		
		...
		def update(self, canonical=None, ttl=None):
			...
			if resp.status_code != 200:
                                try:
                                        return self.infoblox_.__caller__('Could not update CNAME record for {0} - Status {1}'.format(self.name,resp.status_code), resp.status_code)
                                except:
                                        return resp.status_code
                        return 0
